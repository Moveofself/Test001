...
namespace TestForm
{
    public class XmlHelper
    {
        ...
        public static void GetXmlValueToDataSet(string xml_string, ref DataSet ds)
        {
        ...
        /// <returns></returns>
        public static string WriteXML<T>(T item, string path, string fileName, string extension)
        {
            ...
            object[] obj = new object[] { path, "\\", fileName, extension};
        ...
        /// <param name="ends">结束符号（整个xml的路径类似如下：C:\xmltest\201111send.xml，其中path=C:\xmltest,jjdbh=201111,ends=send）</param>
        /// <returns></returns>
        public static string WriteXML<T>(T item, string fullFileName)
        {
            int i = 0;//控制写入文件的次数，
            XmlSerializer serializer = new XmlSerializer(item.GetType());
            ...
            while (true)
            {
                try
                {
                    ...
                    fs.Close();
                    TextWriter writer = new StreamWriter(fullFileName, false, Encoding.UTF8);
                    XmlSerializerNamespaces xml = new XmlSerializerNamespaces();
                    xml.Add(string.Empty, string.Empty);
                    serializer.Serialize(writer, item, xml);
                    writer.Flush();
        ...
        {
            //Assembly[] GetAssemblies = AppDomain.CurrentDomain.GetAssemblies();

            Assembly tAssembly = Assembly.GetAssembly(typeof(T));
        ...
        {
            XmlReader xmlReader = XmlReader.Create(new StringReader(RemoveAllXmlNamespace(xmlOfObject)), new XmlReaderSettings());
